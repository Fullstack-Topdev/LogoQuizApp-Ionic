...
@Component({
  selector: 'burst-quiz-page',
  templateUrl: './burst-quiz.page.html',
  styleUrls: [
    './burst-quiz.page.scss'
  ]
})
export class BurstQuizPage implements OnInit {
  ...
  ionViewWillEnter() {
    ...
    console.log(this.api.quiz_questions);
  ...
  start_Quiz(){
    ...
    if (this.sel_question.question_type=='muliplechoice'){
        this.answer_list= this.sel_question.answers;
    }else if (this.sel_question.question_type=='hangman'){
        this.answer_list=[];
    }
  ...
  init_data() {
    ...
    if (this.sel_question.question_type=='hangman'){
  ...
  click_refresh(){
    ...
    if (this.api.user_info.refreshs > 0) {
      this.api.user_info.refreshs = this.api.user_info.refreshs - 1;
       ...
       var sel_id = Math.floor(Math.random() * this.api.burstmode_count);
       

       this.zone.run(() => {
          this.sel_question= this.api.quiz_questions[sel_id];
  ...
  finish_level(success_flag){
    ...
    console.log(this.current_questionid);
    ...
    this.router.navigate(['burst-quizresult'], navData);
